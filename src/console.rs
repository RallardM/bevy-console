...
/// Internal completion cache used by the console UI.
...
impl ConsoleCache {
    /// Number of currently available prediction entries.
    pub fn predictions_len(&self) -> usize {
        self.predictions_cache.len()
    }

    /// Whether the current buffer has a matching prediction and should hide suggestion popup.
    pub fn prediction_matches_buffer(&self) -> bool {
        self.prediction_matches_buffer
    }
}
...
pub struct ConsoleState {
    ...
    pub buf: String,
    /// Complete scrollback/log history
    pub scrollback: Vec<String>,
    /// Command input history
    pub history: VecDeque<String>,
    /// Current index in history (0 = current input, 1+ = previous commands)
    pub history_index: usize,
    /// Index of currently selected suggestion (None = no suggestion selected)
